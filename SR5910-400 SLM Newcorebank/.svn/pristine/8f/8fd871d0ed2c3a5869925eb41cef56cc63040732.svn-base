    ...
    public class CreateActivityLogDataAccess
    {
        ...
        public decimal CreateActivityLog(CreateActivityLogData log, string systemCode)
        {
            using (KKCASModel db = new KKCASModel())
            {
                ...
                {
                    var nc = db.CAS_SEARCH_NON_CUSTOMER.Where(n => n.NON_CUSTOMER_ID == log.NoncustomerID && n.SUBSCRIPTION_ID == log.SubscriptionID && n.SUBSCRIPTION_TYPE_ID == log.SubscriptionTypeID).FirstOrDefault();
                    if (nc == null)
                    {
                ...
                if (!String.IsNullOrEmpty(log.SubscriptionID))
                {
                    ...
                    if (sub != null)
                    {
                        ...
                        if (log.SubscriptionTypeID != 0) sub.SUBSCRIPTION_TYPE_ID = log.SubscriptionTypeID;
                    }
                    else
                    {
