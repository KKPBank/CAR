            ...
            try
            {
                ...
                foreach (BatchCARInsertStatusTextFileEntity file in fileList)
                {
                    ...
                    if (validTextFormat.IsValid == false)
                    {
                        ...
                        fileList[f].totalRecord = validTextFormat.TotalRecord;
                    }
                    f += 1;
                }

                //4. Process Text File ตาม Requirement
                if (fileList != null && fileList.Where(a => a.totalRecord>0).Any())
                {
                    _logger.Info(_logMsg.Clear().SetPrefixMsg("Start Process Textfile").ToInputLogString());

                    List<CARInsertStatusTextfileDataList> dataList = new List<CARInsertStatusTextfileDataList>();
                    Task.Factory.StartNew(() => Parallel.ForEach(fileList.Where(a => a.totalRecord > 0), new ParallelOptions { MaxDegreeOfParallelism = WebConfig.GetTotalCountToProcess() },
                            file =>
                            {
                                lock (sync)
                                {
                                    this.IncrementSeqCounter();
                                    BulkInsertStatusRequest reqBatch = _batchFacade.LoadBatchCARInsertStatusTextData(file.filePath, file.totalRecord);
